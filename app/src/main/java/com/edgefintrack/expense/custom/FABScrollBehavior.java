...
public class FABScrollBehavior extends FloatingActionButton.Behavior {
    ...
    public boolean onStartNestedScroll(CoordinatorLayout coordinatorLayout,
        ...
        if (target instanceof NestedScrollView && target.getTag() != null) {
            if (target.getTag().toString().equalsIgnoreCase(EdgeExpensesApp.getContext().getString(R.string.statistics)) || target.getTag().toString().equalsIgnoreCase(EdgeExpensesApp.getContext().getString(R.string.history))) {
                return false;
            }
        }
