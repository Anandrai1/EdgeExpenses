...
import com.edgefintrack.expense.adapters.CategoriesSpinnerAdapter;
...
public class NewExpenseFragment extends DialogFragment implements View.OnClickListener {
    ...
    static NewExpenseFragment newInstance(@IUserActionsMode int mode, String expenseId, int mExpenseType) {
        NewExpenseFragment newExpenseFragment = new NewExpenseFragment();
        Bundle bundle = new Bundle();
        bundle.putInt(IUserActionsMode.MODE_TAG, mode);
        bundle.putInt("mExpenseType", mExpenseType);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (getArguments() != null) {
            mExpenseType = getArguments().getInt("mExpenseType");
        }
        return rootView;
    }
    ...
    private void setModeViews() {
        ...
        List<Category> categoriesList = Category.getCategoriesForType(mExpenseType);
    ...
    private void onSaveExpense() {
        Toast.makeText(getContext(), "" + mExpenseType, Toast.LENGTH_SHORT).show();
        if (mCategoriesSpinnerAdapter.getCount() > 0) {
            if (!Util.isEmptyField(etTotal)) {
                ...
                if (mUserActionMode == IUserActionsMode.MODE_CREATE) {
                    //Toast.makeText(getContext(), "MODE_CREATE", Toast.LENGTH_SHORT).show();
                    RealmManager.getInstance().save(new Expense(description, selectedDate, mExpenseType,null,currentCategory, Float.parseFloat(total)), Expense.class);
                } else {
                    //Toast.makeText(getContext(), "MODE_UPDATE", Toast.LENGTH_SHORT).show();
